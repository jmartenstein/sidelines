...
def display_team_games(games, season, team_abbr):
    ...
    for game in games:
        ...
        if game['home_score'] is not None and game['away_score'] is not None:
            score = f"{home} {int(game['home_score'])} - {away} {int(game['away_score'])}"
        else:
            score = "N/A"
...
def main():
    parser = argparse.ArgumentParser(description="List NFL games for a team in a season.")
    parser.add_argument("season", type=int, help="The season year (e.g., 2023)")
    parser.add_argument("team", type=str, help="The 3-letter team abbreviation (e.g., KC)")
